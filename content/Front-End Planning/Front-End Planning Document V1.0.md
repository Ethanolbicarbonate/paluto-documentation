...
> **Version:** 1.0 | **Stack:** Next.js 14+ (App Router), TypeScript, Tailwind CSS, shadcn/ui, React Query, Zustand **Audience:** Front-end development team. This document is the authoritative implementation roadmap.
...
│   │       ├── branches/
│   │       │   ├── page.tsx              # /admin/branches (list)
│   │       │   └── [branchId]/
│   │       │       └── page.tsx          # /admin/branches/[id] (detail)
│   │       ├── affiliates/
...
│   │   ├── BranchFilter.tsx              # Dropdown: Bitoon | Passi | HQ | All
...
│   ├── transaction.types.ts              # Transaction, FoodStampType
...
|`/admin/branches`|`admin/branches/page.tsx`|Admin only|List of all branches (Bitoon, Passi, HQ) as cards with quick metrics.|
|`/admin/branches/[branchId]`|`admin/branches/[branchId]/page.tsx`|Admin only|Drill-down into a single branch: its transactions, affiliates, and commission summary.|
...
**Layout:** `AffiliateMobileLayout` — Full-height mobile viewport. Fixed bottom navigation bar (4 icons: Dashboard, Earnings, Meals, Leaderboard). Fixed top header with greeting and driver code. No sidebar. Optimized for 375px–430px viewports.
...
- Standard HTML table (not paginated — only 3 branches).
...
- A vertical list of 5 items. Each item: `Rank Medal Icon | Avatar (initials) | Name + Driver Code | Branch Badge | Total Earnings`.
- Footer: `"View Full Leaderboard →"` link to `/admin/leaderboard`.

**`RevenueLineChart`**

- Recharts `<LineChart>` with 3 lines (one per branch, different colors).
- X-axis: date buckets based on DWM filter (daily = hours, weekly = days, monthly = weeks).
- Y-axis: revenue in Philippine Peso (₱).
- Tooltip on hover shows all 3 branch values.
- Legend below chart.
...
- `PageHeader`: Branch name (e.g., "Bitoon Branch"), location string, `<DWMFilter>`.
...
- `<SearchBar>` — search by name or driver code
- `<BranchFilter>` — dropdown: All | Bitoon | Passi | HQ
...
- Sortable columns: `Name | Driver Code | Type | Affiliation Group | Branch | Manager | Commission Rate | Total Earned | Status`.
...
- Three cards rendered in 2-1-3 visual order (1st in center, elevated). Each shows: Rank Medal, Avatar, Name, Driver Code, Total Earned, Trip Count.
...
- **`AffiliateSearchCombobox`** — A combobox input (type-to-search). User types a name or driver code (e.g., "D-0055"). Calls `GET /api/manager/affiliates/search?q=` (debounced 300ms). Dropdown shows matching results: `[Driver Code] — [Full Name] — [Group Badge]`. On selection:
    - Locks the field to show selected affiliate.
    - Auto-populates a read-only display below: `Contact Number: [phone]`, `Affiliation Group: [group]`, `Commission Rate: [rate]`.
...
- **Referral Success** — `<Toggle>`. Default: ON (true). Label: "Customer was referred by this affiliate."
...
- **Was a Food Stamp/Voucher Given?** — `<Toggle>`. Default: OFF (false).
    - **If toggled ON:** A conditional field `<FoodStampConditional>` slides in below (CSS `transition-all` for smooth reveal):
        - **Food Stamp Type** — `<Select>` dropdown. Options: `Standard`, `Unli Paluto & Sugba Converted`, `[future types from DB]`.
        - This field is required if `food_stamp = true`. Validated on submit.

**Commission Preview (Live Calculation):**

- Below Section 2, after revenue is entered and affiliate is selected: a subtle highlighted box appears.
- Label: "Estimated Commission"
- Value: auto-calculated (see Section 4 of this document for formula).
- Updates live as `total_revenue` is typed.
- Marked `"(Preview — saved on submit)"` in smaller text.
...
2. `POST /api/manager/transactions` with body: `{ affiliateId, branchId, headcount, totalRevenue, transactionDate, referralSuccess, freeMealClaimed, foodStamp, foodStampType, notes }`.
3. API creates the `transactions` row, then immediately creates the `commission_wallet` row (see Section 4 for calculation).
...
- `<SearchBar>` — search by affiliate name or driver code.
...
**`AffiliateTable`** — Columns: `Code | Name | Type | Group | Contact | Commission Rate | Total Earned | Banked Meals | Actions`.
...
- **Header:** Avatar (initials), full name, driver code, `<RoleBadge>`, affiliation group, contact number.
...
- Commission Rate — `<Input type="number">` with `%` suffix. Tooltip: "The percentage of total_revenue paid as commission." Required.
- Driver Code — Auto-suggested by system (`D-XXXX` for drivers, `G-XXXX` for guides, sequentially). Manager can override. Shown as editable input with generated suggestion.
- Notes (optional textarea).

Submission → `POST /api/manager/affiliates`. Creates `users` record, `affiliates` record, links `manager_id` from session.
...
**`CommissionTable`** — Columns: `Affiliate | Driver Code | Trip Date | Revenue | Commission | Status | Actions`.
...
All affiliate pages share the `AffiliateMobileLayout`: fixed header showing `"Hi, [First Name] 👋"` and `"[Driver Code]"`, and a bottom tab bar with icons for: Home, Earnings, Meals, Leaderboard.
...
**When a Manager submits a new transaction, the commission amount is auto-calculated server-side.**

**Rule:** The `commission_rate` stored on the `affiliates` table is a **percentage** (e.g., `10` = 10%).

**Formula:**

```
commission_wallet.amount = transactions.total_revenue * (affiliates.commission_rate / 100)
```
...
- `commission_wallet.amount = 2500 * (10 / 100) = ₱250.00`

**Server-side API Logic (`POST /api/manager/transactions`):**

```ts
// 1. Receive form data
const { affiliateId, branchId, headcount, totalRevenue, transactionDate,
        referralSuccess, freeMealClaimed, foodStamp, foodStampType, notes } = body;

// 2. Fetch affiliate's commission_rate
const affiliate = await db.affiliates.findUnique({ where: { id: affiliateId } });

// 3. Calculate commission
const commissionAmount = parseFloat(totalRevenue) * (parseFloat(affiliate.commission_rate) / 100);

// 4. Insert transaction
const transaction = await db.transactions.create({
  data: { affiliateId, branchId, headcount, totalRevenue, transactionDate,
          referralSuccess, freeMealClaimed, foodStamp,
          foodStampType: foodStamp ? foodStampType : null, notes }
});

// 5. Insert commission_wallet entry (status defaults to 'Pending')
await db.commissionWallet.create({
  data: { transactionId: transaction.id, amount: commissionAmount, status: 'Pending' }
});

// 6. Return success + the new transaction + commission record
```

**Live Preview on Front-End (`TransactionForm`):**

```ts
// Inside TransactionForm component — updates as user types
const commissionPreview = useMemo(() => {
  if (!selectedAffiliate || !totalRevenue) return null;
  return parseFloat(totalRevenue) * (parseFloat(selectedAffiliate.commissionRate) / 100);
}, [selectedAffiliate, totalRevenue]);
```
...
WHERE t.branch_id = :branchId
  AND t.free_meal_claimed = FALSE
GROUP BY a.id, a.first_name, a.last_name, a.driver_code, a.affiliation_group, a.type
...
WHERE a.branch_id = :branchId
  AND (
    a.driver_code ILIKE :q
    OR a.first_name ILIKE :q
    OR a.last_name ILIKE :q
    OR CONCAT(a.first_name, ' ', a.last_name) ILIKE :q
  )
ORDER BY a.driver_code ASC
LIMIT 10;
```

- Query param `:q` is formatted as `%searchTerm%` before the query.
- Response is used to populate the combobox dropdown.
- On selection, `contact_number`, `affiliation_group`, and `commission_rate` are used to auto-populate read-only display fields and the live commission preview.
...
// { dates: ['2024-01-01', ...], series: { Bitoon: [1200, ...], Passi: [800, ...] } }
```

---

### 4.12 Driver Code Auto-Generation

When the Manager opens the `AffiliateForm` to onboard a new affiliate, the system suggests the next sequential driver code:

**`GET /api/manager/affiliates/next-code?type=driver`**

```sql
-- Get the highest existing numeric suffix for the given type prefix
SELECT driver_code
FROM affiliates
WHERE driver_code ILIKE 'D-%'   -- or 'G-%' for tour_guide
ORDER BY driver_code DESC
LIMIT 1;
-- Result: "D-0054"
...
_End of Paluto Affiliate Tracking System — Front-End Planning Document v1.0_