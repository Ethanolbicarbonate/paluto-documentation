...
> **Version:** 1.1 | **Stack:** Next.js 14+ (App Router), TypeScript, Tailwind CSS, shadcn/ui, React Query, Zustand **Audience:** Front-end development team. This document is the authoritative implementation roadmap.
> 
...
> - **§4.9 + §4.16 (new)** Driver code normalization: all reads/writes go through `.toUpperCase()`. Case-insensitive matching enforced at both the API and DB layer.
...
- `<BranchFilter>` — dropdown: All | Bitoon | Passi
...
- Driver Code — Auto-suggested by system (`D-XXXX` for drivers, `G-XXXX` for guides, sequentially). Manager can override. Shown as editable input with generated suggestion. **Value is always saved as uppercase** (see §4.16).
...
**Driver code normalization is mandatory here.** The input `"d-0055"` and `"D-0055"` must resolve to the same affiliate. Normalize both the search query and the stored DB value.
...
WHERE a.branch_id = :branchId
  AND (
    UPPER(a.driver_code) LIKE :queryParam        -- e.g., '%D-005%'
...
### 4.16 Driver Code Normalization — Full Standard

Driver codes must be stored, searched, and displayed in a **consistent uppercase format** system-wide. A driver using `"d-0055"` on a physical card, a manager searching `"D-0055"`, and the record in the DB reading `"D-0055"` must all be treated as identical.
...
// Usage examples:
normalizeDriverCode('d-0055')  // → 'D-0055'
normalizeDriverCode('  G-0012 ') // → 'G-0012'
normalizeDriverCode('g0012')   // → 'G0012' (valid if that's the format)
```
...
|---|---|
|`AffiliateForm` (onboard)|`normalizeDriverCode(formData.driverCode)` before `POST /api/manager/affiliates`|
|`AffiliateForm` (edit)|Same — normalize before `PATCH` request|
|API `POST /api/manager/affiliates`|`driver_code: normalizeDriverCode(body.driverCode)` before DB insert|
...
|Location|Where to Apply|
|---|---|
|`AffiliateSearchCombobox` (client)|`normalizeDriverCode(inputValue)` before debounced API call|
|`GET /api/manager/affiliates/search`|`normalizeDriverCode(searchParams.get('q'))` before SQL query|
|SQL `WHERE` clause|`UPPER(a.driver_code) LIKE :param` — never rely on exact-case match in DB|
|`TransactionTable` display|Display `a.driver_code` as-is (already normalized in DB)|

**Auto-generation of new codes (reviewed from §4.12):**

```ts
// api/manager/affiliates/next-code/route.ts
...
const latest = await db.$queryRaw`
  SELECT driver_code FROM affiliates
  WHERE UPPER(driver_code) LIKE ${prefix + '-%'}
  ORDER BY driver_code DESC
  LIMIT 1
`;

// Parse the numeric suffix and increment
const latestCode = latest[0]?.driver_code ?? `${prefix}-0000`;
const currentNum = parseInt(latestCode.split('-')[1], 10);
const nextCode = `${prefix}-${String(currentNum + 1).padStart(4, '0')}`;
// e.g., "D-0055" → "D-0056"

return NextResponse.json({ suggestedCode: nextCode });
```

**Zod validator for driver code format:**
...
export const driverCodeSchema = z
  ...
  .trim()
  .toUpperCase()
  .regex(/^[DG]-\d{4}$/, 'Driver code must be in format D-0001 or G-0001');
```

---

_End of Paluto Affiliate Tracking System — Front-End Planning Document v1.1_