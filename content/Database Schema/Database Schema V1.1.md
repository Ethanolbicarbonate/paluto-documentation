...
Table affiliates {
  id                int            [pk, increment]
  user_id           int            [unique, not null, note: 'Links to users table for mobile app login']  
  manager_id        int            [note: 'The manager who onboarded them — nullable, set null on manager delete']
  branch_id         int            [not null, note: 'Their primary branch']                              
...
Table branches {
  id         int          [pk, increment]
  name       varchar      [not null, note: 'Changed from enum to allow dynamic creation']  
  location   varchar
  created_at timestamp    [default: `now()`]
}
...
Table transactions {
  id                int             [pk, increment]
  affiliate_id      int             [not null, note: 'Links to affiliates.id — required']           
  branch_id         int             [not null]                                                       
  headcount         int             [not null, note: 'Total headcount — must be at least 1']        
  total_revenue     decimal         [not null, note: 'Trip Amount/Total Bill — must be greater than 0']  
  transaction_date  timestamp       [not null, note: 'When the customer actually visited']          
  ...
  food_stamp_type   varchar         [null, note: 'Filled ONLY if food_stamp is true']              
...
Table commission_wallet {
  id             int               [pk, increment]
  transaction_id int               [not null, note: 'FK to transactions — multiple wallet entries allowed for split commissions']  
  amount         decimal           [not null, note: 'Calculated Earnings per Trip — must be zero or positive']                     
...
// Operational Links
Ref "onboards":           affiliate_managers.id < affiliates.manager_id [delete: set null]
...
Before the tables, the database uses two "Enums" (lists of allowed values) to keep status data consistent and prevent typos:
...
| **`driver_id`** | Unique Text. The visual ID printed on their physical ID card (e.g., `D-0055`). This is what managers type into the search bar. |
...
| :--- | :--- |
| **`id`** | Primary Key. Auto-increments. |
| **`name`** | Text. The name of the branch (e.g., `Bitoon`, `Passi`, `HQ`). |
